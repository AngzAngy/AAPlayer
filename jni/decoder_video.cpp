...
DecoderVideo::DecoderVideo(AVStream* stream) : IDecoder(stream),userData(NULL)
{
...
//						 packet->size);
    ...
    avcodec_decode_video2(mStream->codec, mFrame, &got_frame, packet);
	...
	if (got_frame) {
	    __android_log_print(ANDROID_LOG_INFO, TAG, "decoding video3333");
		pts = synchronize(mFrame, pts);

		onDecode(mFrame, pts, userData);
