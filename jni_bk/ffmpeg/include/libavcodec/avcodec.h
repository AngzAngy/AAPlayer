...
#define FF_MB_DECISION_RD     2        ///< rate distortion
     ...
     * AVFrame.[] must also be set, but it should be the same as
     ...
     *     callback must allocate and fill  to contain all pointers
     *     to all data planes. data[] must hold as many pointers as it can.
     *      must be allocated with av_malloc() and will be freed in
     *     av_frame_unref().
     *   * otherwise exended_data must point to data
     * - buf[] must contain one or more pointers to AVBufferRef structures. Each of
     *   the frame's data and  pointers must be contained in these. That
 ...
 *
 * AVFrame  channel pointers are allocated if necessary for
 * planar audio.
 *
